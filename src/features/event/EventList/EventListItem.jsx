...
import EventListPeople from './EventListPeople';
...
class EventListItem extends Component {
  render () {
    ...
    return (
     ...
     <Segment>
       <span>
         <Icon name="clock" />{format(event.date.toDate(), 'ddd Do MMMM')} at {format(event.date.toDate(), 'HH:mm')} |
     ...
     <Segment secondary>
       <List horizontal>
         {event.attendees && objectToArray(event.attendees).map((attendee) => (
           <EventListPeople key={attendee.id} attendee={attendee} />
         ))}
       </List>
     </Segment>
     <Segment clearing>
